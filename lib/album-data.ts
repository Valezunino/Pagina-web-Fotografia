  ...
  try {
    ...
    return albumRows.flatMap((album) => {
      ...
      return [{
        ...
        coverImage: `/api/photos/${albumPhotos[0].id}/preview?wv=3`,
      }];
    });
  } catch {
    return [];
  ...
  try {
    ...
    return {
      album: {
        ...
        coverImage: `/api/photos/${rows[0].id}/preview?wv=3`,
      },
      photos: rows.map((photo) => ({ ...photo, image: `/api/photos/${photo.id}/preview?wv=3` })),
    };
  } catch {
    return null;
  }
